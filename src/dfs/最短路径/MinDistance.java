...
public class MinDistance {
   ...
   /*度小满的最短路径的问题*/
    ...
    int[][] dp;
    int m, n;//尺寸
    int ans = Integer.MAX_VALUE;//记录的问题
    public void dfs(int x, int y, int value) {
        if (value > dp[x][y] || value >= ans || grid[x][y] == '#') {
            return;//dp记录了值，value要的是最小的。grid的去除。
        }
        ...
        dfs(x + 1, y, value + (grid[x + 1][y] == '*' ? 1 : 0));
        dfs(x - 1, y, value + (grid[x - 1][y] == '*' ? 1 : 0));
        dfs(x, y + 1, value + (grid[x][y + 1] == '*' ? 1 : 0));
        dfs(x, y - 1, value + (grid[x][y - 1] == '*' ? 1 : 0));
    }
}
